...
pub type Event {
  ...
  // all newlines are recorded, can occur in or outside of a tag, but we don't support multi-line attrs so it will be outside of an attr value
  ...
  // "<!DOCTYPE " or "<!Doctype " or "<!doctype "
  TagStartClosing(blame: Blame, load: String)

  // "</name"
  InTagWhitespace(blame: Blame, load: String)

  // whitespace that occur outside of a tag go directly into Text
  ...
  // "<!--" CommentStart sequences that occur inside of tag are considered errors and are ignored for the purpose of keeping in-tag/out-tag state
  CommentEndSequence(blame: Blame)
  // "-->"  CommentEnd sequences that occur without a matching CommentStart have no effect on state, either (of course)
}
...
fn advance_line(cl: ContentLine, by: Int) -> ContentLine {
  ...
  ContentLine(bl.advance(cl.blame, by), string.drop_start(cl.content, by))
}
...
fn check_for_tag_after_lt(after: String) -> TagOrNot {
  ...
  use <- on.true_false(before == "?xml" || before == "?XML", fn() {
  ...
  use <- on.true_false(is_ordinary_tag(before), fn() { Ordinary(before) })
  NoTag
}
...
fn check_for_tag_after_lt_closing(after: String) -> TagOrNot {
  let s = sp.new([" ", ">", "/>", "?>"])
  let #(before, _, _) = sp.split(s, after)
  use <- on.true_false(is_ordinary_tag(before), fn() { OrdinaryClosing(before) })
  NoTag
}

fn take_text_up_to_next_tag(text: String) -> #(String, TagOrNot) {
  use #(text, after) <- on.error_ok(string.split_once(text, "<"), fn(_) {
    #(text, NoTag)
  })

  use <- on.true_false(string.starts_with(after, "/"), fn() {
...
fn event_stream_internal(
  ...
  use <- on.true_false(first.content == "", fn() {
    case rest {
      [] -> previous |> list.reverse
      _ ->
        event_stream_internal([Newline(first.blame), ..previous], state, rest)
    }
  })

  // true_false
  use <- on.true_false(state == OutsideTag, fn() {
    ...
    use <- on.true_false(tag_or_not == NoTag, fn() {
    ...
    let #(tag_event, z, tag, new_state) = case tag_or_not {
      XMLDoc(tag) -> #(
        TagStartXMLVersion(end_of_text_blame, tag),
        "<?",
        tag,
        InsideOpeningTagExpectingNextKey,
      )
      Doctype(tag) -> #(
        TagStartDoctype(end_of_text_blame, tag),
        "<!",
        tag,
        InsideOpeningTagExpectingNextKey,
      )
      Ordinary(tag) -> #(
        TagStartOrdinary(end_of_text_blame, tag),
        "<",
        tag,
        InsideOpeningTagExpectingNextKey,
      )
      OrdinaryClosing(tag) -> #(
        TagStartClosing(end_of_text_blame, tag),
        "</",
        tag,
        InsideClosingTag,
      )
      CommentStart -> #(
        CommentStartSequence(end_of_text_blame),
        "<!--",
        "",
        InsideComment,
      )
    ...
    case string.length(first.content) < length {
      True -> {
        let msg =
          "tag split invariant failed: content="
          <> ins(first.content)
          <> ", text="
          <> ins(text)
          <> ", prefix="
    ...
    event_stream_internal([tag_event, ..previous], new_state, [
      advance_line(first, length),
      ..rest
    ])
  })

  use <- on.true_false(state == InsideComment, fn() {
    case string.split_once(first.content, "-->") {
      ...
      Ok(#(before, _)) -> {
        case before == "" {
          True -> {
            event_stream_internal(
              [CommentContents(first.blame, before), ..previous],
              OutsideTag,
  ...
  use <- on.true_false(num_whitespace > 0, fn() {
  ...
  // ...get rid of '=' sign
  use <- on.true_false(string.starts_with(first.content, "="), fn() {
    event_stream_internal(
