...
fn closing_equals_opening(pair: LatexDelimiterPair) -> Bool {
  let z = core.opening_and_closing_singletons_for_pair(pair)
  z.0 == z.1
}
...
fn rrs_split_pair_fold_data(
  ...
  case which {
    DoubleDollarSingleton ->
      RRSSplitPairFoldData(
        grs.unescaped_suffix_rr_splitter(
          re_suffix: "\\$\\$",
          replacement: grs.Tag("DoubleDollar"),
        ),
        "DoubleDollar",
    ...
    EndAlign ->
      RRSSplitPairFoldData(
        grs.rr_splitter(
          re_string: "\\\\end{align}",
          replacement: grs.TextAndTag("EndAlign", "\\end{align}"),
        ),
        "EndAlign",
        "\\end{align}",
      )
    ...
    EndAlignStar ->
      RRSSplitPairFoldData(
        grs.rr_splitter(
          re_string: "\\\\end{align\\*}",
          replacement: grs.TextAndTag("EndAlignStar", "\\end{align*}"),
        ),
        "EndAlignStar",
        "\\end{align*}",
      )
    ...
    BeginEnvironment(name) ->
      RRSSplitPairFoldData(
        grs.rr_splitter(
          re_string: "\\\\begin{" <> regex_escape_environment_name(name) <> "}",
          replacement: grs.TagAndText(
            "BeginEnv:" <> name,
            "\\begin{" <> name <> "}",
          ),
    ...
    EndEnvironment(name) ->
      RRSSplitPairFoldData(
        grs.rr_splitter(
          re_string: "\\\\end{" <> regex_escape_environment_name(name) <> "}",
          replacement: grs.TextAndTag(
            "EndEnv:" <> name,
            "\\end{" <> name <> "}",
          ),
        ),
...
fn split_pair_fold_for_delimiter_pair(
  ...
  case closing_equals_opening(pair) {
    True -> {
      ...
      [
        dl.naive_unescaped_split_and_replace__outside(#(s, e, r), forbidden),
        dl.pair(#(tag, tag, wrapper, unbridgeable)),
        dl.fold_into_text(#(tag, s)),
      ]
    }
    False -> {
      let RRSSplitPairFoldData(rrs1, tag1, replacement1) =
        rrs_split_pair_fold_data(d1)
      let RRSSplitPairFoldData(rrs2, tag2, replacement2) =
        rrs_split_pair_fold_data(d2)
      [
...
fn create_math_or_mathblock_elements(
  ...
  let delims = case which {
    "MathBlock" -> core.latex_strippable_display_delimiters()
    "Math" -> core.latex_inline_delimiters()
...
pub fn symmetric_delim_splitting(
  ...
  let opening_or_closing_grs =
    grs.rr_splitter_for_groups([
      #("[^\\s]|^", grs.Keep),
      #(
        grs.unescaped_suffix(delim_regex_form),
        grs.Tag("OpeningOrClosingSymmetricDelim"),
      ),
      #("[^\\s\\]})]|$", grs.Keep),
    ])
...
pub fn markdown_link_splitting(
  ...
  let start_splitter =
    grs.unescaped_suffix_rr_splitter("\\[", grs.Tag(start_tag))
  [
