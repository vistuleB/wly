...
fn nodemap(
  ...
  case vxml {
    T(_, _) -> Ok(vxml)
    V(_, tag, attrs, _) -> case string.starts_with(tag, "__"), core.attrs_have_key(attrs, "had_href_tag") {
      True, _ -> Error(core.DesugaringError(vxml.blame, "check_proper_detokenization found tag: " <> tag))
      False, True -> Error(core.DesugaringError(vxml.blame, "had_href_attr still there"))
      _, _ -> Ok(vxml)
    }
  }
}
