...
fn word_node(blame: Blame, word: String) {
  V(blame, "__OneWord", [Attr(desugarer_blame(284), "val", word)], [])
}
  ...
  case vxml {
    V(_, _, attrs, children) -> {
      case core.attrs_have_key(attrs, "had_href_child") {
        False -> vxml
        True -> {
          let children = list.fold(inner, children, fn(acc, pair) { match_until_end(acc, pair.0, pair.1) })
  ...
  |> list.index_map(
    ...
    fn(x, i) {
      case i % 2 == 0 {
        ...
        False -> {
          assert string.starts_with(x, "_") && string.ends_with(x, "_") && string.length(x) > 2
