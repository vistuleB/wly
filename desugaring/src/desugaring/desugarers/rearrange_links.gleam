  ...
  case children {
    ...
    [first, ..rest] -> {
      case first {
        ...
        V(_, _, _, children) -> case core.v_has_attr_with_key(first, "href") {
          True -> {
    ...
    [p, ..pattern_rest] -> {
      case p {
        StartT -> generate_replacement_vxml_internal(
          [start_node(desugarer_blame(120)), ..already_ready],
          pattern_rest,
          match_data,
        )

        EndT -> generate_replacement_vxml_internal(
          [end_node(desugarer_blame(126)), ..already_ready],
          pattern_rest,
          match_data,
        )

        Space -> generate_replacement_vxml_internal(
          [space_node(desugarer_blame(132)), ..already_ready],
          pattern_rest,
          match_data,
        )

        Word(word) -> generate_replacement_vxml_internal(
          [word_node(desugarer_blame(138), word), ..already_ready],
          pattern_rest,
          match_data,
        )
...
fn fast_forward_past_spaces(
  ...
  list.drop_while(atomized, core.v_tag_is_one_of(_, ["__OneSpace", "__OneNewLine"]))
}
...
fn match_internal(
  ...
  case pattern {
    ...
    [A(_, _, href_int, pattern_internal), ..pattern_rest] -> {
      case atomized {
        [V(_, _, _, children) as v, ..atomized_rest] -> case core.v_has_attr_with_key(v, "href") {
          True -> {
...
fn word_node(blame: Blame, word: String) {
  V(blame, "__OneWord", [Attr(desugarer_blame(356), "val", word)], [])
}
...
fn tokenize_if_t_or_has_href_tag_recursive(opening_punctuation_splitter: Splitter, vxml: VXML) -> List(VXML) {
  case vxml {
    ...
    V(_, _, _, children) -> case core.v_has_attr_with_key(vxml, "href") {
      True -> {
        let children = list.flat_map(children, fn(vxml) {tokenize_if_t_or_has_href_tag_recursive(opening_punctuation_splitter, vxml)})
...
fn tokenize_maybe( opening_punctuation_splitter: Splitter,children: List(VXML)) -> Option(List(VXML)) {
  case list.any(children, core.is_v_and_has_attr_with_key(_, "href")) {
    True -> {
      children
      |> list.map(fn(vxml) { tokenize_if_t_or_has_href_tag_recursive(opening_punctuation_splitter, vxml) })
      |> list.flatten
      |> Some
    }
    False -> None
  ...
  case vxml {
    V(_, _, _, children) -> {
      use atomized <- on.eager_none_some(
        tokenize_maybe(inner.2, children),
        vxml,
      )
...
fn vxml_to_link_pattern(
  ...
  case vxml {
    ...
    V(_, tag, attrs, children) -> {
      ...
      use classes <- on.ok(
        case core.attrs_with_key(attrs, "class") {
          [] -> Ok("")
          [one] -> Ok(one.val)
...
fn param_to_inner_param(param: Param) -> Result(InnerParam, DesugaringError) {
  ...
  Ok(#(pattern1, pattern2, opening_punctuation_splitter))
}
...
type InnerParam = #(LinkPattern, LinkPattern, Splitter)
