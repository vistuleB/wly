...
fn load_cut_nodes(vxml: VXML) -> Dict(String, List(VXML)) {
  ...
  let assert Ok(cut_nodes) = list.find(children, core.is_v_and_tag_equals(_, "NodesBeingMoved"))
  ...
  list.fold(
    ...
    fn(acc, node) {
      ...
      let assert Ok(#(Some(attr), attrs)) = core.attrs_extract_unique_key_or_none(attrs, "chapter")
...
fn v_before_transforming_children(
  vxml: VXML,
  state: State,
) -> Result(#(VXML, State, List(DesugaringWarning), core.TrafficLight), DesugaringError) {
  ...
  use _ <- on.stay(case tag {
    ...
    "Appendix" -> case core.attrs_val_first_with_key(attrs, "handle") {
      // Appendix >>exercise-graveyard we Return & Continue:
      Some("exercise-graveyard") -> on.Return(Ok(#(vxml, state, [], core.Continue)))
...
fn v_after_transforming_children(
  ...
  case original_state.0 {
    False -> {
      let assert V(_, "GrandWrapper", _, children) = vxml
      let assert Ok(book) = list.find(children, core.is_v_and_tag_equals(_, "Book"))
