...
fn elements_for_line(line: Line) -> List(VXML) {
  ...
  case before == terminal_prompt {
    ...
    True -> [
      terminal_prompt_span |> core.v_prepend_child(line_2_t(Line(line.blame, before))),
      terminal_prompt_content_span |> core.v_prepend_child(line_2_t(Line(after_blame, after))),
    ]
  }
}
      ...
      case core.v_has_key_val(vxml, "language", "arbitrary-prompt-response") {
        True -> {
          ...
          V(
            ...
            |> core.attrs_append_classes(desugarer_blame(102), "arbitrary-prompt-response"),
            children,
          )
        }
        _ -> vxml
