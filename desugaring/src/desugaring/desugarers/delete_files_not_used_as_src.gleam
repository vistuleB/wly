...
fn nodemap(vxml: VXML, state: State, inner: InnerParam) -> State {
  ...
  use <- on.eager_false_true(list.contains(inner.3, tag), state)
...
fn at_root(root: VXML, inner: InnerParam) -> Result(Nil, DesugaringError) {
  ...
  let extensions = list.map(inner.2, core.ensure_prefix(_, "."))
  ...
  paths
  |> list.map(core.assert_drop_prefix(_, inner.0 <> "/"))
  |> list.filter(should_be_deleted)
  |> list.try_each(fn(p) {
    let cmd = "rm " <> inner.0 <> "/" <> p
    io.println(whoami <> ": " <> cmd)
    shellout.command(run: "rm", with: [inner.0 <> "/" <> p], in: ".", opt: [])
    |> result.map_error(fn(e) {
      DesugaringError(
        desugarer_blame(74),
        "failed to delete " <> inner.0 <> "/" <> p <> " (" <> ins(e) <> ")",
      )
    })
  })
}
...
fn param_to_inner_param(param: Param) -> Result(InnerParam, DesugaringError) {
  Ok(#(
    param.0 |> core.drop_suffix("/"),
    param.1 |> core.drop_suffix("/"),
    case param.2 {
...
type Param =
  #(
    String,
    // exec_dir_to_build_dir
    String,
    // build_dir_to_build_img_dir
    List(String),
    // delete only files with these extensions
    List(String),
    // if no tag in this list ever mentions that src
  )
