...
fn v_before_transforming_children(
  ...
  state: State(a),
  inner: InnerParam(a),
) -> Result(#(VXML, State(a), List(DesugaringWarning), core.TrafficLight), DesugaringError) {
  let #(a, attrs) = state
...
fn v_before_1(_: Nil, vxml: VXML) -> Result(#(Nil, List(Attr), List(DesugaringWarning), core.TrafficLight), DesugaringError){
  let assert V(_, tag, attrs, _) = vxml
  use <- on.false_true(
    tag == "In",
...
fn v_before_2(_: Nil, vxml: VXML) -> Result(#(Nil, List(Attr), List(DesugaringWarning), core.TrafficLight), DesugaringError){
