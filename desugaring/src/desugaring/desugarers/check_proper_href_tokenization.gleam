      ...
      case has_href(vxml), list.any(children, is_v_and_has_href) {
        False, False -> {
          case list.any(children, is_v_and_has_tag_starting_with(_, "__")) {
            True -> Error(core.DesugaringError(vxml.blame, "found tokenization error (1)"))
            False -> Ok(vxml)
          }
        }
        False, True -> {
          case remaining_properly_tokenized(False, children), core.attrs_have_key(attrs, "had_href_child") {
            True, True -> Ok(vxml)
            False, True -> Error(core.DesugaringError(vxml.blame, "found tokenization error (2a)"))
            True, False -> Error(core.DesugaringError(vxml.blame, "found tokenization error (2b)"))
            False, False -> Error(core.DesugaringError(vxml.blame, "found tokenization error (2c)"))
          }
        }
        True, False -> {
          case remaining_properly_tokenized(False, children) {
            True -> Ok(vxml)
            False -> Error(core.DesugaringError(vxml.blame, "found tokenization error (3)"))
          }
        }
